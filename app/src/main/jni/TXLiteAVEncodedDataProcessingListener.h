...
#ifndef APP_ANDROID_TRTC_TRTCDEMO_MAIN_JNI_TXLITEAVENCODEDDATAPROCESSINGLISTENER_H_
#define APP_ANDROID_TRTC_TRTCDEMO_MAIN_JNI_TXLITEAVENCODEDDATAPROCESSINGLISTENER_H_
...
class ITXLiteAVEncodedDataProcessingListener {
 public:
  ...
  /**
   * 回调解码前的视频数据。
   *  @note videoData.userId 表示对应的user，当userId 为
   * nullptr时，表示此时先接收到数据了，对应的userId还未完成同步。获取到userId之后会回调正确的userId
   */
  ...
  /**
   * 回调解码前的音频数据。
   *  @note audioData.userId 表示对应的user，当userId 为
   * nullptr时，表示此时先接收到数据了，对应的userId还未完成同步。获取到userId之后会回调正确的userId
   */
...
#endif  // APP_ANDROID_TRTC_TRTCDEMO_MAIN_JNI_TXLITEAVENCODEDDATAPROCESSINGLISTENER_H_
